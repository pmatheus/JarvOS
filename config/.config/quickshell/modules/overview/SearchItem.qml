...
RippleButton {
    ...
    property var itemExecuteShift: entry?.executeShift
    property var itemExecuteCtrl: entry?.executeCtrl
    property var itemExecuteAlt: entry?.executeAlt
    ...
    function executeWithModifiers(modifiers) {
        if ((modifiers & Qt.ShiftModifier) && root.itemExecuteShift) {
            root.itemExecuteShift();
        } else if ((modifiers & Qt.ControlModifier) && root.itemExecuteCtrl) {
            root.itemExecuteCtrl();
        } else if ((modifiers & Qt.AltModifier) && root.itemExecuteAlt) {
            root.itemExecuteAlt();
        } else {
            root.itemExecute();
        }
    ...
    onClicked: {
        executeWithModifiers(0)
    }
    Keys.onPressed: (event) => {
        if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter) {
            root.keyboardDown = true
            executeWithModifiers(event.modifiers)
            event.accepted = true;
        }
    }
