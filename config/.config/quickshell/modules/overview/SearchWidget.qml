...
Item { // Wrapper
    ...
    property var searchActions: [
        ...
        },
        {
            action: "dark",
            execute: () => {
                executor.executeCommand(`current_wallpaper=$(awww query | head -1 | sed 's/.*image: //' | tr -d '\\n\\r'); ${Directories.wallpaperSwitchScriptPath} "$current_wallpaper" --mode dark`)
            }
        },
        {
            action: "light",
                    ...
                    Keys.onPressed: (event) => {
                        if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter) {
                            if (appResults.count > 0) {
                                ...
                                if (firstItem) {
                                    firstItem.executeWithModifiers(event.modifiers);
                                    event.accepted = true;
                                }
                            }
                        }
            ...
            ListView { // App results
                ...
                model: ScriptModel {
                    ...
                    values: { // Search results are handled here
                        ...
                        //////////////// Files & Folders //////////////////
                        result = result.concat(
                            FileSearch.fuzzyQuery(root.searchingText)
                                .map((entry) => {
                                    const dirPath = entry.isDir ? entry.path : entry.path.replace(/\/[^/]*$/, "");
                                    return {
                                        name: entry.name,
                                        clickActionName: qsTr("Open"),
                                        type: entry.isDir ? qsTr("Folder") : qsTr("File"),
                                        materialSymbol: entry.isDir ? 'folder_open' : 'description',
                                        execute: () => {
                                            Qt.openUrlExternally(`file://${entry.path}`);
                                        },
                                        executeShift: () => {
                                            executor.executeCommand(`nautilus "${dirPath}"`);
                                        },
                                        executeCtrl: () => {
                                            executor.executeCommand(`antigravity "${entry.path}"`);
                                        },
                                        executeAlt: () => {
                                            executor.executeCommand(`kitty --directory "${dirPath}"`);
                                        },
                                    };
                                })
                        );
